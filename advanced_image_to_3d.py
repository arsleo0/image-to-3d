#!/usr/bin/env python3
"""
Advanced Image-to-3D Pipeline for Commercial 3D Printing
Converts product photos to printable STL files with business analytics

Fixed version with:
- Automatic directory creation
- Updated trimesh API (non-deprecated)
- Proper error handling and verification for STL export
...
# Try to import rembg (optional - may not work on Python 3.14)
try:
...
def ensure_directories(config: dict):
    ...
    directories = [
        ...
        paths.get('processing_dir', 'processing'),
        paths.get('stl_dir', 'output/stl'),
        paths.get('previews_dir', 'output/previews'),
        paths.get('reports_dir', 'output/reports'),
        'logs'
    ]
    ...
    def detect(self) -> dict:
        ...
        if cuda_available and self.config.get('auto_detect_gpu', True):
            ...
            if self.config.get('cpu_fallback', True):
                ...
                self.logger.warning("No GPU detected, using CPU (processing will be slower)")
            else:
...
class ImagePreprocessor:
    ...
    def center_and_pad(self, image: Image.Image) -> Image.Image:
        ...
        try:
            ...
            if channels == 1:
                ...
                padded = np.zeros((new_h, new_w, channels), dtype=np.uint8)
    ...
    def process(self, image_path: Path) -> Optional[Image.Image]:
        ...
        # Remove background
        if self.config.get('background_removal', True) and REMBG_AVAILABLE:
...
class PrintingOptimizer:
    ...
    def check_wall_thickness(self, mesh: trimesh.Trimesh) -> bool:
        ...
        if len(mesh.edges) > 0:
            ...
            if min_edge < min_thickness:
                self.logger.warning(
                    f"Minimum edge length ({min_edge:.2f}mm) is below recommended "
    ...
    def fix_mesh(self, mesh: trimesh.Trimesh) -> trimesh.Trimesh:
        """Fix common mesh issues using updated trimesh API"""
        try:
            # Fill holes
            ...
            # Remove duplicate and degenerate faces using updated API
            # Instead of deprecated remove_duplicate_faces() and remove_degenerate_faces()
            unique_faces = mesh.unique_faces()
            if len(unique_faces) < len(mesh.faces):
                mesh.update_faces(unique_faces)
                self.logger.debug(f"Removed {len(mesh.faces) - len(unique_faces)} duplicate faces")

            nondegenerate = mesh.nondegenerate_faces()
            if len(nondegenerate) < len(mesh.faces):
                mesh.update_faces(nondegenerate)
                self.logger.debug(f"Removed {len(mesh.faces) - len(nondegenerate)} degenerate faces")
...
class BusinessCalculator:
    ...
    def calculate_weight(self, volume_mm3: float, density: float = 1.24) -> float:
        """Calculate weight in grams (default PLA density: 1.24 g/cm3)"""
    ...
    def estimate_print_time(self, mesh: trimesh.Trimesh) -> float:
        ...
        try:
            ...
            if len(mesh.edges) > 0:
                ...
                perimeter = np.mean(edge_lengths) * len(mesh.edges) / 100
            else:
                perimeter = 100  # default estimate
    ...
    def process(self, mesh: trimesh.Trimesh, category: str = 'default') -> dict:
        ...
        self.logger.info(f"   Volume: {metrics['volume_mm3']:.2f} mm3")
        self.logger.info(f"   Weight: {metrics['weight_g']:.2f} g")
        self.logger.info(f"   Material Cost: EUR {metrics['material_cost_eur']:.2f}")
        self.logger.info(f"   Recommended Price: EUR {metrics['recommended_price_eur']:.2f}")
...
class Exporter:
    ...
    def export_stl(self, mesh: trimesh.Trimesh, output_path: Path) -> bool:
        """Export mesh as STL file with proper error handling"""
        try:
            # CRITICAL: Ensure output directory exists
            output_dir = output_path.parent
            output_dir.mkdir(parents=True, exist_ok=True)
            self.logger.debug(f"Ensured directory exists: {output_dir}")
            ...
            if not output_path.exists():
                self.logger.error(f"STL export failed: File was not created at {output_path}")
                return False

            file_size = output_path.stat().st_size
            if file_size == 0:
                self.logger.error(f"STL export failed: File is empty at {output_path}")
            ...
            self.logger.error(f"Permission denied when saving STL to {output_path}: {e}")
            return False
        except OSError as e:
            self.logger.error(f"OS error when saving STL to {output_path}: {e}")
            return False
        except Exception as e:
...
class Image3DPipeline:
    ...
    def __init__(self, config_path: str = 'config.yaml'):
        ...
        # CRITICAL: Ensure all directories exist before processing
    ...
    def process_single(self, input_path: Path, category: str = 'default') -> Optional[dict]:
        ...
        try:
            ...
            plt.imsave(str(depth_vis_path), depth_map, cmap='viridis')
            ...
            export_success = self.exporter.export_stl(mesh_trimesh, stl_path)

            if not export_success:
                self.logger.error(f"Failed to export STL for {input_path.name}")
                return None
            ...
            self.logger.info(f"STL saved to: {stl_path}")
            self.logger.info(f"{'='*60}\n")
...
class BatchProcessor:
    ...
    def generate_batch_report(self, results: List[dict], failed: List[str]):
        ...
        try:
            ...
            self.logger.info(f"Total material cost: EUR {total_cost:.2f}")
            self.logger.info(f"Total retail value: EUR {total_price:.2f}")
            self.logger.info(f"Potential profit: EUR {total_price - total_cost:.2f}")
            self.logger.info(f"{'='*60}\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Advanced Image-to-3D Pipeline for Commercial 3D Printing'
    )
    parser.add_argument(
        '--config',
        default='config.yaml',
    ...
    try:
        ...
        if args.input:
            ...
            if result:
                print(f"\nSuccess! STL saved: {result.get('stl_file', result['sku'] + '.stl')}")
            else:
                print("\nProcessing failed")
                sys.exit(1)
            ...
            print("Please specify --input for single image or --batch for batch processing")
            parser.print_help()
            sys.exit(1)
